...
"""
    ...
    # Agrega la parte del encabezado con el título dado
    html_content += "<h2>{}</h2><table>".format(title)

    # Agrega cada fila en data como una fila en la tabla
    # La primera línea es especial y se trata por separado
...
def write_html_file(html_string, html_file):
    ...
    # Verifica si el archivo html que estamos escribiendo ya existe o no
...
def main():
    """Verifica los argumentos de línea de comandos y luego llama a la función de procesamiento"""
    # Verifica que se incluyan los argumentos de línea de comandos
    if len(sys.argv) < 3:
